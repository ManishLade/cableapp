...
@Component({
    ...
    entryComponents: [AddManageAgentsComponent, EditManageAgentsComponent, DeleteDialogComponent]
})
export class ManageAgentsComponent implements OnInit {
    ...
    addManageAgents() {
        this.router.navigate(['/manage-agents/add'], {
    ...
    deleteItem(i: number, row: any) {
        ...
        dialogRef.afterClosed().subscribe((confirmed: boolean) => {
            if (confirmed) {
                self.dataService.deleteManageAgents(row.Id).subscribe(res => {
                    if (res['Result']) {
    ...
    onEdit(index: number, manageAgents: ManageAgents) {
        const navigationExtras: NavigationExtras = { state: manageAgents, relativeTo: this.route } as NavigationExtras;
        this.router.navigate(['/manage-agents/edit'], navigationExtras);
    }

    private refreshTable() {
        // Refreshing table using paginator
        // Thanks yeager-j for tips
        // https://github.com/marinantonio/angular-mat-table-crud/manageAgentss/12
...
export class ExampleDataSource extends DataSource<ManageAgents> {
    ...
    filteredData: ManageAgents[] = [];
    renderedData: ManageAgents[] = [];
    ...
    connect(): Observable<ManageAgents[]> {
        ...
        return merge(...displayDataChanges).pipe(
            map(() => {
                // Filter data
                this.filteredData = this._exampleDatabase.data.slice().filter((manageAgents: ManageAgents) => {
                    const searchStr = (manageAgents.Name + manageAgents.Id).toLowerCase();
    ...
    sortData(data: ManageAgents[]): ManageAgents[] {
