...
export class FranchiseeComponent implements OnInit {
    ...
    addFranchisee() {
        this.router.navigate(['/franchisee/add'], {
    ...
    deleteItem(i: number, row: any) {
        ...
        dialogRef.afterClosed().subscribe((confirmed: boolean) => {
            if (confirmed) {
                self.dataService.deleteFranchisee(row.Id).subscribe(res => {
                    if (res['Result']) {
    ...
    onEdit(index: number, franchisee: Franchisee) {
      const navigationExtras: NavigationExtras = { state: franchisee, relativeTo: this.route } as NavigationExtras;
      this.router.navigate(['/franchisee/edit'], navigationExtras);
    }

    private refreshTable() {
        // Refreshing table using paginator
        // Thanks yeager-j for tips
        // https://github.com/marinantonio/angular-mat-table-crud/franchisees/12
