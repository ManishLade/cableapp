...
@Component({
    ...
    styleUrls: ['./channel-category.component.scss'],
...
export class ChannelCategoryComponent implements OnInit {
    ...
    addChannelCategory() {
        this.router.navigate(['/channel-category/add'], {
    ...
    deleteItem(i: number, row: any) {
        ...
        dialogRef.afterClosed().subscribe((confirmed: boolean) => {
            if (confirmed) {
                self.dataService.deleteChannelCategory(row.Id).subscribe(res => {
                    if (res['Result']) {
    ...
    onEdit(index: number, channelCategory: ChannelCategory) {
        const navigationExtras: NavigationExtras = { state: channelCategory, relativeTo: this.route } as NavigationExtras;
        this.router.navigate(['/channel-category/edit'], navigationExtras);
    }

    private refreshTable() {
        // Refreshing table using paginator
        // Thanks yeager-j for tips
        // https://github.com/marinantonio/angular-mat-table-crud/channelCategorys/12
...
export class ExampleDataSource extends DataSource<ChannelCategory> {
    ...
    filteredData: ChannelCategory[] = [];
    renderedData: ChannelCategory[] = [];
    ...
    connect(): Observable<ChannelCategory[]> {
        ...
        return merge(...displayDataChanges).pipe(
            map(() => {
                // Filter data
                this.filteredData = this._exampleDatabase.data.slice().filter((channelCategory: ChannelCategory) => {
                    const searchStr = (channelCategory.Name + channelCategory.Id).toLowerCase();
    ...
    sortData(data: ChannelCategory[]): ChannelCategory[] {
