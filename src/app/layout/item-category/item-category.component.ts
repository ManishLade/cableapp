...
export class ItemCategoryComponent implements OnInit {
    ...
    addItemCategory() {
        this.router.navigate(['/item-category/add'], {
    ...
    deleteItem(i: number, row: any) {
        ...
        dialogRef.afterClosed().subscribe((confirmed: boolean) => {
            if (confirmed) {
                self.dataService.deleteItemCategory(row.Id).subscribe(res => {
                    if (res['Result']) {
    ...
    onEdit(index: number, itemCategory: ItemCategory) {
      const navigationExtras: NavigationExtras = { state: itemCategory, relativeTo: this.route } as NavigationExtras;
      this.router.navigate(['/item-category/edit'], navigationExtras);
    }

    private refreshTable() {
        // Refreshing table using paginator
        // Thanks yeager-j for tips
        // https://github.com/marinantonio/angular-mat-table-crud/itemCategorys/12
